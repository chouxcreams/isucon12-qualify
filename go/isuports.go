...
func Run() {
	...
	jwkPem, _, err := jwk.DecodePEM(keysrc)
	if err != nil {
...
func tenantsBillingHandler(c echo.Context) error {
	...
	var beforeID int64 = 9223372036854775807
	if before != "" {
		var err error
...
func playerHandler(c echo.Context) error {
	...
	if err := tenantDB.SelectContext(
		...
		"SELECT ps.ID AS ctitle, ps.score AS pscore FROM player_score AS ps JOIN (SELECT competition_id, MAX(row_num) AS newest FROM player_score GROUP BY competition_id HAVING player_id = ? AND tenant_id = ?) AS ps2 ON ps.competition_id = ps2.competition_id AND ps.row_num = newest LEFT JOIN competition AS c ON c.id = ps.competition_id  WHERE ps.tenant_id =? AND ps.player_id = ? ",
		p.ID,
		v.tenantID,
		v.tenantID,
		p.ID,
