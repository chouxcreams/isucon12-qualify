...
func playerHandler(c echo.Context) error {
	...
	if err := tenantDB.SelectContext(
		...
		"SELECT c.title AS ctitle, ps.score AS pscore FROM player_score AS ps JOIN (SELECT competition_id, MAX(row_num) AS newest FROM player_score GROUP BY competition_id) AS ps2 ON ps.competition_id = ps2.competition_id AND ps.row_num = ps2.newest LEFT JOIN competition AS c ON c.competition_id = ps.competition_id WHERE ps.tenant_id = ? AND ps.player_id = ?",
		v.tenantID,
		p.ID,
	); err != nil {
